<template>
  <div class="monitor-realtime-page">
    <!-- 3D场景区域 -->
    <div class="scene-container">
      ...
      <div class="device-control-panel">
        <div class="panel-header">
          <h3><el-icon><Monitor /></el-icon> 设备控制</h3>
          <div class="header-actions">
            <el-button 
              ...
              @click="togglePanel"
            >
              <el-icon>{{ isPanelCollapsed ? 'DArrowRight' : 'DArrowLeft' }}</el-icon>
            </el-button>
          </div>
        </div>
...
const handleDeviceHover = (device: DeviceData | null) => {
  ...
  if (device && hoveredDevice.value) {
    // 模拟计算悬浮框位置（实际应该根据3D坐标转换）
    popupPosition.x = 100
    popupPosition.y = 100
    
    // TODO: 实际应该根据设备在屏幕上的位置计算
    // const screenPos = cesiumCoordinateConverter.wgs84ToScreen(device.position)
    // popupPosition.x = screenPos[0] + 10
    // popupPosition.y = screenPos[1] - 10
  }
}
